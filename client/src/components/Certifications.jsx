...
export default function Certifications() {
  ...
  const getPlatformBadge = (platform) => {
    ...
    if (plat === 'hackerrank') {
      return {
        name: 'HackerRank',
        icon: <FontAwesomeIcon icon={byPrefixAndName.fab['hackerrank']} className="text-sm text-emerald-400" />,
        badgeClasses: 'text-emerald-400 bg-emerald-950/50 border-emerald-500/40 shadow-sm shadow-emerald-950',
        cardBorderHover: 'hover:border-emerald-500/50 hover:shadow-emerald-500/10',
        viewLinkAccent: 'text-emerald-400 hover:text-emerald-300',
      };
    }
    ...
    if (plat === 'simplilearn') {
      return {
        name: 'Simplilearn',
        icon: <FontAwesomeIcon icon={byPrefixAndName.fas['graduation-cap']} className="text-xs text-blue-400" />,
        badgeClasses: 'text-blue-400 bg-blue-950/50 border-blue-500/40 shadow-sm shadow-blue-950',
        cardBorderHover: 'hover:border-blue-500/50 hover:shadow-blue-500/10',
        viewLinkAccent: 'text-blue-400 hover:text-blue-300',
      };
    }
    ...
    if (plat === 'forage') {
      return {
        name: 'Forage',
        icon: <FontAwesomeIcon icon={byPrefixAndName.fas['briefcase']} className="text-xs text-purple-400" />,
        badgeClasses: 'text-purple-400 bg-purple-950/50 border-purple-500/40 shadow-sm shadow-purple-950',
        cardBorderHover: 'hover:border-purple-500/50 hover:shadow-purple-500/10',
        viewLinkAccent: 'text-purple-400 hover:text-purple-300',
      };
    }
    ...
    if (plat === 'coursera') {
      return {
        ...
        badgeClasses: 'text-cyan-400 bg-cyan-950/50 border-cyan-500/40 shadow-sm shadow-cyan-950',
        cardBorderHover: 'hover:border-cyan-500/50 hover:shadow-cyan-500/10',
        viewLinkAccent: 'text-cyan-400 hover:text-cyan-300',
      };
    }
    ...
    if (plat === 'ibm') {
      return {
        name: 'IBM',
        icon: <Award className="w-3.5 h-3.5 text-sky-400" />,
        badgeClasses: 'text-sky-400 bg-sky-950/50 border-sky-500/40 shadow-sm shadow-sky-950',
        cardBorderHover: 'hover:border-sky-500/50 hover:shadow-sky-500/10',
        viewLinkAccent: 'text-sky-400 hover:text-sky-300',
      };
    }
    ...
    if (plat === 'aws') {
      return {
        ...
        badgeClasses: 'text-amber-400 bg-amber-950/50 border-amber-500/40 shadow-sm shadow-amber-950',
        cardBorderHover: 'hover:border-amber-500/50 hover:shadow-amber-500/10',
        viewLinkAccent: 'text-amber-400 hover:text-amber-300',
      };
    }
    ...
    return {
      name: platform,
      icon: <Award className="w-3.5 h-3.5 text-cyan-400" />,
      badgeClasses: 'text-slate-300 bg-slate-900/70 border-slate-700/60',
      cardBorderHover: 'hover:border-cyan-500/50 hover:shadow-cyan-500/10',
      viewLinkAccent: 'text-cyan-400 hover:text-cyan-300',
    };
  };
    ...
    <section id="certificates" className="relative py-24 px-4 sm:px-6 lg:px-8 max-w-6xl mx-auto">
      ...
      <div className="flex flex-col items-center text-center mb-12">
        ...
        <h2 className="text-3xl sm:text-4xl font-extrabold text-white tracking-tight">
          Certi<span className="bg-gradient-to-r from-cyan-400 to-blue-500 bg-clip-text text-transparent">ficates</span>
        </h2>
          ...
          <button
            ...
            className={`px-4 py-1.5 rounded-full text-xs sm:text-sm font-medium transition-all duration-200 ${
              selectedPlatform === plat
                ...
                : 'bg-[#0c1527]/70 text-slate-400 hover:text-white border border-slate-800 hover:border-cyan-500/30'
            }`}
          >
            {plat}
          </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredCerts.map((cert) => {
          ...
          return (
            <div
              key={cert.id}
              className={`group bg-[#0c1527]/70 border border-slate-800 rounded-xl overflow-hidden ${platBadge.cardBorderHover} transition-all duration-300 flex flex-col shadow-lg hover:-translate-y-1`}
            >
              ...
              {certImage ? (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="relative w-full h-48 sm:h-52 bg-white overflow-hidden border-b border-slate-800 flex items-center justify-center block group/img cursor-pointer"
                >
                  <img
                ...
                <div className="relative w-full h-44 bg-gradient-to-br from-slate-900 via-[#0a1020] to-[#0d182e] p-4 flex flex-col justify-between border-b border-slate-800/80 overflow-hidden group-hover:from-slate-900 group-hover:to-[#0f203d] transition-colors">
                  ...
                  <div className="absolute inset-3 border border-slate-700/50 rounded-lg pointer-events-none"></div>
                  ...
                  <div className="relative z-10 flex flex-col items-center justify-center my-auto text-center px-4">
                    <div className="w-10 h-10 rounded-full bg-cyan-500/10 border border-cyan-500/30 flex items-center justify-center text-cyan-400 mb-1 group-hover:scale-110 transition-transform">
                      <FileCheck className="w-5 h-5" />
                    </div>
                    <div className="text-[11px] font-medium text-slate-400 line-clamp-1">
                      {certIssuer}
                    </div>
                  </div>
                  ...
                  <div className="relative z-10 flex items-center justify-between text-[11px]">
                    <span className="px-2 py-0.5 rounded bg-slate-800/90 text-slate-300 font-mono font-medium border border-slate-700">
                      {certYear}
                    </span>
              ...
              <div className="p-5 flex flex-col flex-1">
                <div className="flex items-start gap-3 mb-2">
                  ...
                  <div>
                    <h3 className="font-bold text-white text-base sm:text-[15px] group-hover:text-cyan-300 transition-colors line-clamp-2 leading-snug">
                      {certTitle}
                    </h3>
                ...
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className={`${platBadge.viewLinkAccent} text-sm font-semibold flex items-center gap-1.5 mt-auto pt-4 border-t border-slate-800/60 transition-colors group/link`}
                >
